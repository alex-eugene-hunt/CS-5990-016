...
public:
    ...
    void update(const std::vector<double>& state, const std::vector<double>& action, double reward, const std::vector<double>& nextState) {
        std::string stateKey = stateToString(state);
        std::string actionKey = stateToString(action);
        double qPredict = qTable[stateKey][actionKey];
        double qTarget = reward + gamma * getMaxQ(nextState);
        qTable[stateKey][actionKey] += alpha * (qTarget - qPredict);
    }

    std::vector<double> chooseAction(const std::vector<double>& state) {
...
private:
    ...
    double gamma;
    double epsilon;
    std::unordered_map<std::string, std::unordered_map<std::string, double>> qTable;
    ...
    double getMaxQ(const std::vector<double>& state) {
        std::string stateKey = stateToString(state);
        double maxQ = 0.0;
        for (const auto& action : qTable[stateKey]) {
    ...
    std::vector<double> getRandomAction() {
        std::vector<double> action(10);
        for (double& val : action) {
            val = (double) std::rand() / RAND_MAX;
        }
        return action;
    }

    std::vector<double> getBestAction(const std::string& stateKey) {
        double maxQ = 0.0;
        std::vector<double> bestAction(10, 0.0);
        for (const auto& action : qTable[stateKey]) {
            if (action.second > maxQ) {
                maxQ = action.second;
                bestAction = stringToState(action.first);
            }
        }
        return bestAction;
    }

    std::vector<double> stringToState(const std::string& str) {
        std::vector<double> state;
        size_t start = 0;
        size_t end = str.find("_");
        while (end != std::string::npos) {
            state.push_back(std::stod(str.substr(start, end - start)));
            start = end + 1;
...
QLearning qLearning(0.1, 0.9, 0.1);
...
bool TrajSolver::solve(const PVAJ &initState,
    ...
    // Get initial state and action from Q-learning
    std::vector<double> state = { initState.position.norm(), endState.position.norm(),
                                  (endState.position - initState.position).norm(),
                                  initState.velocity.norm(), endState.velocity.norm(),
                                  initState.acceleration.norm(), endState.acceleration.norm() };

    std::vector<double> action = qLearning.chooseAction(state);

    // Set initial guess based on action
